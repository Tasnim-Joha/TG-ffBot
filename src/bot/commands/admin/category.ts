...
export async function addCategory(ctx: Context) {
  ...
  try {
    ...
    logger.info({ admin: ctx.from?.id, name }, 'Category added');
    await ctx.reply(`✅ Category "${name}" created.`);
  } catch (error) {
    logger.error(error, 'Add category error');
    await ctx.reply('❌ Failed to create category.');
  }
}

// /editcategory <id> <new_name> [new_description]
export async function editCategory(ctx: Context) {
  // Check if the message is a text message
  if (!ctx.message || !('text' in ctx.message)) {
  ...
  try {
    ...
    logger.info({ admin: ctx.from?.id, id, name }, 'Category edited');
    await ctx.reply(`✅ Category updated.`);
  } catch (error) {
    logger.error(error, 'Edit category error');
    await ctx.reply('❌ Failed to update category.');
  }
}

// /deletecategory <id>
export async function deleteCategory(ctx: Context) {
  // Check if the message is a text message
  if (!ctx.message || !('text' in ctx.message)) {
  ...
  try {
    ...
    const productsInCat = await db.select().from(products).where(eq(products.categoryId, id));
    if (productsInCat.length > 0) {
      await ctx.reply('❌ Cannot delete category with products. Delete products first.');
      return;
    }
    await db.delete(categories).where(eq(categories.id, id));
    logger.info({ admin: ctx.from?.id, id }, 'Category deleted');
